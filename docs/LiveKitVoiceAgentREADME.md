...
| Warm transfer to a human agent | **Working — Plan 6.9–6.10c complete (2026-09-13).** `transfer_call` announces to the caller, dials the PBX destination, whispers the seven-field summary to the human only, then bridges. `transfer_status` and timings land on the call row. No-answer/busy/reject/fail speak a fallback and never leave silence. After bridge the AI stops talking; recording continues; human speech is `HUMAN_AGENT`. Seed: **Reception** ext. `1000`, Development Agent has transfer enabled. Verify by listening to both legs. |
...
| Entering a provider API key and testing it in the UI | **Working — AI Setup (Plan 4c complete).** Dedicated `/ai-setup` page with four tabs; test-before-save enforced; 33 providers across LLM/STT/TTS/Embedding after Plan 6.11. Endpoint URL is now pre-populated on provider selection and saved to DB (Plan 4c.8, 2026-09-13). 43 standard voices seeded for cloud TTS providers; publish no longer blocked by missing voice when provider has no catalog voices (Plan 4c.9, 2026-09-13). See [`AIProviders.md`](./AIProviders.md). |
...
| Caller heard the summary whisper | Audio isolation bug — the whisper must be on `agent-whisper` with human-only subscribe permissions. Treat as a release blocker; confirm `transfer_track_permissions_set` and that the caller was unsubscribed from the human. |
| Caller heard silence during transfer | Announcement text empty, or the hold loop failed. Publish requires announcement text when transfer is enabled. Check `transfer_status` (`ANNOUNCING` vs later). |
| Transfer never reaches the human agent | `transfer_status` says which step stalled. `FAILED` with `transfer_dial_missing_trunk` means the destination (or the call's trunk) has no LiveKit resource id — inbound-only trunks cannot place the human leg. `AGENT_NO_ANSWER` / `BUSY` / `REJECTED` should speak the fallback, not drop the caller. |
| Human answered but never heard the summary | Destination `whisper_summary` is off, or summary generation failed and the fallback whisper was skipped. `calls.transfer_summary` still has the seven fields. DTMF `1` (default) skips the whisper on purpose. |
| Call ended when the human declined | Fixed 2026-09-13. `_wait_for_disconnect` used to treat any remote leave as hangup. The human leaving before bridge is now ignored so fallback can run. |
...
| Publish blocked by `voice_id: choose a voice` even though a voice picker shows no options | Voices for this TTS provider have not been registered in the platform catalog. Either: (a) run `seed-platform` again — standard voices for OpenAI TTS, ElevenLabs, Cartesia, Deepgram, Google TTS, LMNT and PlayHT are now seeded automatically; or (b) add voices manually via Platform → Voices. After Plan 4c.9 the publish is only blocked when voices *exist* but none is selected; a provider with no voices at all downgrades to a warning. |
