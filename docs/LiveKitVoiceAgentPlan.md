...
| # | Work item | Spec |
|---|---|---|
| ~~7.1~~ | ~~Helm charts with independent scaling for Frontend, API, LiveKit, LiveKit SIP, AI Workers~~ **Done (2026-09-13):** `deploy/helm/voice-agent-platform/` — three Deployments (configuration-api, ai-agent-worker, frontend) each with their own Service, HPA, and PDB. Sub-chart dependencies on Bitnami postgresql/redis/minio. `Chart.yaml` + `values.yaml` + `values.production.yaml` + `deploy/helm/DEPLOYMENT.md`. LiveKit managed by its own official chart (not in this repo). | §5, §4 |
